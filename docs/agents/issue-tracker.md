...
GitHub shares one number space across issues and PRs, so resolve an explicitly named item with `gh pr view <number>` and fall back to `gh issue view <number>`.
...
- **Blocking**: use GitHub’s native issue dependencies. Where dependencies are unavailable, add `Blocked by: #<n>` to the child body.
