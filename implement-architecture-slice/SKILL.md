...
Run the slice's focused and stress gates plus the repository's ordinary local suite. Commit only scoped files, push, and open one product PR that links the parent and plan, records the plan commit and slice heading, and closes the child issue.

Before the first review, make that same product PR own every committed current-state transition caused by its merge. When the repository records slice completion or dispatchable frontiers in the normative plan, program index, or another committed status surface, update those files in the product PR to mark this slice complete and name only the successors that will be ready after merge. If the repository convention requires the PR number, open the PR first as a draft (`gh pr create --draft`) and then push the status commit before review. Write facts that become true when the PR reaches the default branch; do not predict a merge SHA, journal PR, hosted run, or other unknown receipt. These are product state changes, not journal history, and must not be deferred to a routine docs-only frontier PR.

This step is complete when the intended PR implements every acceptance criterion, includes its committed completion/frontier transition where applicable, and changes no neighboring slice contract.
...
Build every prompt from the exact normative plan slice and the shared [review briefing](../_shared/REVIEW-LOOP.md#review-briefing), following `ras-review`'s mechanical quoting guidance. Link round history without copying chronology into the plan. Before the first review, confirm the candidate contains every applicable committed completion/frontier transition from Step 3; adding one after review changes the reviewed head and requires the composed exact-head gates again. Before exact-head certification, revalidate the predicted frontier against live blocker and merge state; if it changed, correct the product PR and repeat the gates required for its new head.
...
Execute the shared [bounded review algorithm](../_shared/REVIEW-LOOP.md#bounded-review-algorithm). After it and the terminating evidence plan are complete with no unresolved stop, push the final candidate, run every accepted stress gate, and run the repository's exact-head local certification. Mark the draft PR ready only for that certified head, wait for every required `ci-*` check to succeed on the live head (a post-ready `ci` run with conclusion `success`; a draft-phase `skipped` check is not evidence), and squash-merge that exact head with `--match-head-commit`. Diagnose failed CI before deciding whether any rerun is legitimate. If the base advances, repeat every gate required by the repository protocol.
...
**Only after step 4 has merged the main work to the default branch**, run `append-dev-journal` without RAS. **NEVER start `append-dev-journal` before the main work has merged.** Keep the journal change within `append-dev-journal`'s append-only boundary; the product PR already owns committed completion/frontier state, so do not create or plan a routine third PR to advance it. Ensure the child issue is closed, then revalidate every pending `defer` against that merged head per the shared [deferred-finding revalidation](../_shared/REVIEW-LOOP.md#deferred-finding-revalidation) rule — drop the ones this PR already resolved, and cite the merged SHA plus current `file:line` on the rest. Create review follow-up issues only for the survivors, and use `omnifocus-cli` to complete the exact slice task and add those follow-ups.

Recompute only the affected successors against the merged product head and verify that the committed transition predicted in Step 3 is correct. When all of a successor's blockers are closed, apply the repository's agent-ready label, remove its OmniFocus blocked prefix, and refresh the tracking issue and program-parent current frontier without inventing ordering. Keep those mutable mirrors to current state, blockers, and pointers; do not copy contracts, audit history, or exact plan-SHA narratives into them. Do not open a separate frontier/status PR merely to repeat the product PR's state transition. If post-merge evidence invalidates the committed transition, treat that as an exceptional correction: report the mismatch, update live mirrors to facts, and use the repository's authorized corrective path rather than representing it as routine slice closure.
...
The skill is complete when the merged slice, committed current-state surfaces, child issue, journal, OmniFocus tree, successor readiness, and program frontier agree without a routine third frontier PR.
