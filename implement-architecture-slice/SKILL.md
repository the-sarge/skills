...
2. Reconstruct the relevant code and review evidence, including verified fixes and recorded dispositions. Apply the shared [contract-closure reference](../_shared/CONTRACT-CLOSURE.md) only to accepted invariant-and-owner roots involved in the stop.
...
4. Continue only when the same child and one-PR outcome pass those gates with a central enforcement design and a complete bounded closure matrix.
   - Add a durable re-audit section to the plan containing the closure matrix, disposition, trigger evidence, and review/verification IDs. Update the program overview in a clean docs worktree from the plan's authoritative branch: the branch containing the recorded plan commit, usually the default branch unless the program lives on a long-lived handoff branch.
   - Publish through the repository's authorized path and require the resulting commit to be reachable from that authoritative branch before synchronizing its exact SHA into the existing child issue and OmniFocus slice task.
   - Refresh the parent/tracking issues and track/program OmniFocus notes with the revised disposition and frontier. Do not duplicate the commit field where their contract does not define one, and keep re-audit docs out of the implementation diff unless the accepted plan explicitly co-locates them there.
...
Use `tdd` at the seams named by the slice. Make the specified characterization or failing tests red, implement the narrowest contract-satisfying change, and run focused checks regularly. Independently disposition each substantive failure or review finding before designing a fix. Only for a `fix-now` finding, translate the example into its accepted invariant and in-contract sibling family, then fix that family at its central enforcement owner when it fits the accepted contract. Preserve the named single owner, authority boundary, intermediate contract, and non-goals. Report worthwhile deferred improvements instead of absorbing them, and do not create follow-up busywork for rejected or marginal findings.
...
Follow the repository's `docs/REVIEW-LOOP.md` and the shared [review-loop baseline](../_shared/REVIEW-LOOP.md). The shared file owns finding disposition, finding-family analysis, and precise invariant-and-owner classification; the repository protocol may add stronger boundary, certification, CI, merge, journal, tracking, and routing rules. Judge and fix findings yourself; use RAS only as those protocols permit. Route only a `stop-for-decision` finding to the scoped re-audit branch and record all dispositions, the review/verification run IDs, and the precise causal pattern.
...
Before each inner fix loop, record the dispositions required by the shared baseline against the exact slice contract. Only `fix-now` findings enter implementation; route `stop-for-decision` to the scoped re-audit branch, and keep `defer` and `reject` findings out of both paths. For each `fix-now` finding, compare its precise invariant and enforcement owner with prior fresh reviews and verified fixes.

After independent disposition makes a fresh review clean, push the final candidate, run every named stress gate, and run the repository's exact-head local certification. Dispatch hosted CI only for that certified head, verify the live PR head and required result still match, and merge that exact head. Diagnose failed CI before deciding whether any rerun is legitimate. If the base advances, repeat every gate required by the repository protocol.
