...
Execute the shared [bounded review algorithm](../_shared/REVIEW-LOOP.md#bounded-review-algorithm). After it and the terminating evidence plan are complete with no unresolved stop, push the final candidate, run every accepted stress gate, and run the repository's exact-head local certification. Request hosted CI only for that certified head through the shared protocol's PR-associated trigger, verify the live PR head and required result still match, and merge that exact head. Diagnose failed CI before deciding whether any rerun is legitimate. If the base advances, repeat every gate required by the repository protocol.
