...
Choice rule, three ways: a lane that is merge-blocking today **and** needs its own runner or timeout becomes a `ci-<lane>` job; a lane that is merge-blocking on the same runner as `check` does not get a job — fold into `task check`; a lane that is not merge-blocking moves to a [non-required workflow](#non-required-workflows). A lane that only matters for some paths stays conditional by being path-gated inside its target (see the Taskfile contract), not by becoming several jobs.
...
For a private repository, prefer a self-hosted runner label when one exists; otherwise `ubuntu-latest`. Route by job (`ci-<lane>` with its own `runs-on`), never by matrix inside a required job. Count GitHub-hosted runners in required jobs separately: a required job starts its runner on every non-draft same-repository PR run — docs-only changes included, because checkout and setup precede the classifier and only then does the target exit early — so a hosted runner (and especially macOS or Windows, at multiplied minutes) in the required set is billed on every PR regardless of path-gating. Keep required lanes on self-hosted labels; hosted platform matrices belong in `release-gate` or a scheduled workflow unless they are genuinely merge-blocking. `actions/setup-go@v7` and `arduino/setup-task@v3` are Node 24 actions; a self-hosted runner image must be recent enough to run Node 24 or the setup steps fail before `task ci` starts.

No job on a self-hosted runner label uses the GitHub Actions cache — in `ci.yml` or any other workflow. `actions/setup-go` steps in such a job set `cache: false` because that action's cache input defaults to true (the asset ships it; a replacement toolchain setup keeps it); setup actions whose cache input is off by default (`actions/setup-node`, `-python`, `-java`, …) leave it unset — `cache: false` is not a valid value for their string-typed package-manager inputs, and enabling the cache is the deviation — and no `actions/cache` step appears. The portfolio triggers CI only on `pull_request`, so every cache entry is scoped to `refs/pull/N/merge` and is never restored by a later run: the cache is write-only upload cost per job that also fills GitHub's 10-GB per-repository quota, and on a warm self-hosted substrate it re-uploads state the runner already holds locally. Cache locality for self-hosted lanes belongs to the runner substrate (the infra repository's decision 0017), never to GitHub's cache. Hosted-runner jobs may keep the action default. The audit reports `CI-CACHE` / `WF-CACHE` for every job it can classify: a job is self-hosted when any literal `runs-on` label is not a hosted one, even beside expression labels; a job whose only runner evidence is an expression (for example `runs-on: ${{ matrix.runner }}`) cannot be classified at audit time, so the audited guarantee is that canonical subset, not universal.
