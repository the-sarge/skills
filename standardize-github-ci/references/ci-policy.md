...
No job on a self-hosted runner label uses the GitHub Actions cache — in `ci.yml` or any other workflow. Every `actions/setup-*` step in such a job sets `cache: false` (the asset ships it; a replacement toolchain setup keeps it), and no `actions/cache` step appears. The portfolio triggers CI only on `pull_request`, so every cache entry is scoped to `refs/pull/N/merge` and is never restored by a later run: the cache is write-only upload cost per job that also fills GitHub's 10-GB per-repository quota, and on a warm self-hosted substrate it re-uploads state the runner already holds locally. Cache locality for self-hosted lanes belongs to the runner substrate (the infra repository's decision 0017), never to GitHub's cache. Hosted-runner jobs may keep the action default. The audit reports `CI-CACHE` / `WF-CACHE` for violations on literal runner labels; a `runs-on` that resolves at runtime (an expression) cannot be classified at audit time, so the audited guarantee is that canonical subset, not universal.
