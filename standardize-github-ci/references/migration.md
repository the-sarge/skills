...
1. Copy `assets/ci.yml` to `.github/workflows/ci.yml`; set `runs-on`; add `ci-<lane>` jobs by duplicating `ci-required` and changing only the job name, `runs-on`, `timeout-minutes`, the `task ci-<lane>` step, and any job-level `env:`/`services:` the lane needs. For a cross-runner exchange, add `needs:` on the destination `ci-*` jobs only, listing origin `ci-*` jobs by name, and move artifacts with `actions/upload-artifact` / `actions/download-artifact`.
